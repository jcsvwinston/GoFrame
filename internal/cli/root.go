...
func printRootUsage(w io.Writer) {
	...
	fmt.Fprintln(w, "  nucleus serve --config goframe.yaml")
	fmt.Fprintln(w, "  nucleus runserver 0.0.0.0:8080")
	fmt.Fprintln(w, "  nucleus startapp billing --out .")
	fmt.Fprintln(w, "  nucleus migrate --config goframe.yaml status")
	fmt.Fprintln(w, "  nucleus sqlmigrate --migrations migrations 20260401120000_add_users")
	fmt.Fprintln(w, "  nucleus sqlflush --config goframe.yaml")
	fmt.Fprintln(w, "  nucleus flush --config goframe.yaml --yes")
	fmt.Fprintln(w, "  nucleus diffsettings --config goframe.yaml")
	fmt.Fprintln(w, "  nucleus doctor --config goframe.yaml")
	fmt.Fprintln(w, "  nucleus doctor --config goframe.yaml --check tasks")
	fmt.Fprintln(w, "  nucleus doctor --config goframe.yaml --json")
	...
	fmt.Fprintln(w, "  nucleus createcachetable --config goframe.yaml")
	fmt.Fprintln(w, "  nucleus clearsessions --config goframe.yaml")
	fmt.Fprintln(w, "  nucleus remove_stale_contenttypes --config goframe.yaml --dry-run")
	fmt.Fprintln(w, "  nucleus mailproviders --config goframe.yaml")
	...
	fmt.Fprintln(w, "  nucleus makemessages --config goframe.yaml --locale es --input .")
	fmt.Fprintln(w, "  nucleus compilemessages --config goframe.yaml --locale es")
	fmt.Fprintln(w, "  nucleus collectstatic --config goframe.yaml --output public/assets")
	fmt.Fprintln(w, "  nucleus findstatic --config goframe.yaml app.css")
	...
	fmt.Fprintln(w, "  nucleus sendtestemail --config goframe.yaml --to dev@example.com --dry-run")
	fmt.Fprintln(w, "  nucleus check --deploy --config goframe.yaml")
	fmt.Fprintln(w, "  nucleus sqlsequencereset --config goframe.yaml")
	fmt.Fprintln(w, "  nucleus inspectdb --config goframe.yaml --output internal/models/inspected.go")
	fmt.Fprintln(w, "  nucleus ogrinspect --config goframe.yaml --output internal/models/geospatial.go")
	fmt.Fprintln(w, "  nucleus openapi --out openapi.json")
	fmt.Fprintln(w, "  nucleus dumpdata --config goframe.yaml --output fixtures.json")
	fmt.Fprintln(w, "  nucleus loaddata --config goframe.yaml fixtures.json")
	fmt.Fprintln(w, "  nucleus testserver --config goframe.yaml --dry-run fixtures.json")
	fmt.Fprintln(w, "  nucleus changepassword admin --config goframe.yaml --password newsecret123 --no-input")
	fmt.Fprintln(w, "  nucleus showmigrations --config goframe.yaml")
	fmt.Fprintln(w, "  nucleus generate model User")
	fmt.Fprintln(w, "  nucleus test --run TestRun_MigrateLifecycle ./cmd/nucleus")
	fmt.Fprintln(w, "  nucleus routes --config goframe.yaml")
	fmt.Fprintln(w, "  nucleus health --config goframe.yaml")
	fmt.Fprintln(w, "")
