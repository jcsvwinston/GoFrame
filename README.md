...
- `cmd/goframe` + `internal/cli`: CLI modular con comandos `new`, `startapp`, `serve`, `migrate`, `sqlmigrate`, `sqlflush`, `sqlsequencereset`, `flush`, `diffsettings`, `makemessages`, `compilemessages`, `collectstatic`, `findstatic`, `optimizemigration`, `squashmigrations`, `sendtestemail`, `mailproviders`, `inspectdb`, `ogrinspect`, `dumpdata`, `loaddata`, `createcachetable`, `createuser`, `changepassword`, `clearsessions`, `remove_stale_contenttypes`, `seed`, `shell`, `generate`, `test`, `testserver`, `routes`, `health`.
...
go run ./cmd/goframe mailproviders --config goframe.yaml
