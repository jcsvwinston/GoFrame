...
```go
import "github.com/jcsvwinston/GoFrame/pkg/quark/cache/memory"

store := memory.NewStore(10000)
client, _ := quark.New(db,
    quark.WithDialect(quark.SQLite()),
    quark.WithCache(store),
)
...
```go
import "github.com/jcsvwinston/GoFrame/pkg/quark/otel"
