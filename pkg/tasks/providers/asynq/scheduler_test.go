...
func TestSchedulerRegisterAndInspectRuntime(t *testing.T) {
	...
	redisOpt, err := redisClientOptFromURL("redis://" + redisServer.Addr())
	if err != nil {
		t.Fatalf("redisClientOptFromURL failed: %v", err)
	}
...
func TestSchedulerNilReceiver(t *testing.T) {
	...
	if _, err := scheduler.RegisterJSON("@every 1s", "cleanup.sessions", map[string]any{}, tasks.DefaultEnqueuePolicy()); err != ErrNilScheduler {
