...
func TestNewManager_NilLogger(t *testing.T) {
	_, err := NewManager(Config{RedisURL: "redis://localhost:6379/0"}, nil)
	if err != nil {
...
func TestNewManager_RequiresRedisURL(t *testing.T) {
	_, err := NewManager(Config{RedisURL: ""}, nil)
	if err == nil {
...
func TestNewManager_InvalidRedisURL(t *testing.T) {
	_, err := NewManager(Config{RedisURL: "http://invalid"}, nil)
	if err == nil {
...
func TestManager_HandleFuncValidation(t *testing.T) {
	...
	err = (&Manager{}).HandleFunc("  ", func(context.Context, *asynq.Task) error { return nil })
	if err == nil {
...
func TestManager_CloseMultipleTimes(t *testing.T) {
	mgr, err := NewManager(Config{RedisURL: "redis://localhost:6379/0"}, nil)
	if err != nil {
