...
func main() {
	...
	// Register all models with admin panel
	if err := a.RegisterModel(&models.Article{}, model.ModelConfig{
		Icon:         "document",
		ListFields:   []string{"ID", "Title", "Author", "Category", "Published", "CreatedAt"},
		SearchFields: []string{"Title", "Content", "Summary"},
		Filters:      []string{"Published", "Category", "Author"},
		OrderBy:      "created_at desc",
	}); err != nil {
		log.Fatal(err)
	}

	if err := a.RegisterModel(&models.Category{}, model.ModelConfig{
		Icon:         "folder",
		ListFields:   []string{"ID", "Name", "Slug", "ArticleCount"},
	...
	if err := a.RegisterModel(&models.Author{}, model.ModelConfig{
		Icon:         "user",
		ListFields:   []string{"ID", "Name", "Email", "Bio", "ArticleCount"},
		SearchFields: []string{"Name", "Email", "Bio"},
		OrderBy:      "name asc",
	}); err != nil {
		log.Fatal(err)
	}

	if err := a.RegisterModel(&models.Tag{}, model.ModelConfig{
		Icon:         "tag",
		ListFields:   []string{"ID", "Name", "Slug", "Color", "ArticleCount"},
	...
	if err := a.RegisterModel(&models.Comment{}, model.ModelConfig{
		Icon:         "message-circle",
		ListFields:   []string{"ID", "Article", "Author", "Content", "Approved", "CreatedAt"},
		SearchFields: []string{"Content", "Author"},
		Filters:      []string{"Approved"},
		OrderBy:      "created_at desc",
	}); err != nil {
		log.Fatal(err)
	}
	...
	// Parse all templates
	tpl, err := template.ParseGlob("internal/web/templates/*.html")
	if err != nil {
		log.Fatal(err)
	}

	// Configure template engine
	a.Router.SetHTMLTemplates(tpl)
	...
	// Static files
	a.Router.Get("/static/*", func(c *router.Context) error {
		path := c.Request.URL.Path[len("/static/"):]
		return c.File("internal/web/static/" + path)
	})

	// HTML Pages
	a.Router.Get("/", controllers.HomePage(tpl))
	a.Router.Get("/blog", controllers.BlogPage(tpl, sqlDB))
	a.Router.Get("/blog/:id", controllers.ArticlePage(tpl, sqlDB))
	a.Router.Get("/categories", controllers.CategoriesPage(tpl, sqlDB))
	a.Router.Get("/about", controllers.AboutPage(tpl, sqlDB))
	a.Router.Get("/contact", controllers.ContactPage(tpl))
	a.Router.Post("/contact", controllers.ContactSubmit(sqlDB))

	// API Endpoints
	a.Router.Get("/api/health", controllers.Health)
	a.Router.Get("/api/articles", controllers.ListArticles(articleService))
	a.Router.Get("/api/articles/:slug", controllers.GetArticleBySlug(sqlDB))
	a.Router.Get("/api/categories", controllers.ListCategoriesAPI(sqlDB))
	a.Router.Get("/api/authors", controllers.ListAuthorsAPI(sqlDB))
	a.Router.Get("/api/stats", controllers.GetStatsAPI(sqlDB))
	...
	log.Println("========================================")
	log.Println("🚀 Showcase Demo running:")
	log.Println("========================================")
	log.Printf("  🌐 Web:     http://localhost:%d/\n", cfg.Port)
	log.Printf("  📰 Blog:    http://localhost:%d/blog\n", cfg.Port)
	log.Printf("  📂 Categories: http://localhost:%d/categories\n", cfg.Port)
	log.Printf("  ℹ️  About:   http://localhost:%d/about\n", cfg.Port)
	log.Printf("  📧 Contact: http://localhost:%d/contact\n", cfg.Port)
