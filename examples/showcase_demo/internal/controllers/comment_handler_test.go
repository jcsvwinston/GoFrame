...
func TestCommentHandler_CRUDLifecycle(t *testing.T) {
	...
	createRec := performCommentRequest(t, r, http.MethodPost, "/comments/", map[string]any{"name": "Books"})
	if createRec.Code != http.StatusCreated {
		t.Fatalf("expected status %d, got %d", http.StatusCreated, createRec.Code)
	}

	createBody := decodeCommentJSON(t, createRec.Body.Bytes())
	...
	secondCreateRec := performCommentRequest(t, r, http.MethodPost, "/comments/", map[string]any{"name": "Games"})
	if secondCreateRec.Code != http.StatusCreated {
		t.Fatalf("expected status %d, got %d", http.StatusCreated, secondCreateRec.Code)
	}

	listRec := performCommentRequest(t, r, http.MethodGet, "/comments/", nil)
	if listRec.Code != http.StatusOK {
		t.Fatalf("expected status %d, got %d", http.StatusOK, listRec.Code)
	}
	listBody := decodeCommentJSON(t, listRec.Body.Bytes())
	if got := int(listBody["count"].(float64)); got != 2 {
		t.Fatalf("expected list count 2, got %d", got)
	}

	filteredRec := performCommentRequest(t, r, http.MethodGet, "/comments/?q=book", nil)
	if filteredRec.Code != http.StatusOK {
		t.Fatalf("expected status %d, got %d", http.StatusOK, filteredRec.Code)
	}
	filteredBody := decodeCommentJSON(t, filteredRec.Body.Bytes())
	if got := int(filteredBody["count"].(float64)); got != 1 {
		t.Fatalf("expected filtered count 1, got %d", got)
	}

	resourcePath := fmt.Sprintf("/comments/%d", int(resourceID))
	getRec := performCommentRequest(t, r, http.MethodGet, resourcePath, nil)
	if getRec.Code != http.StatusOK {
		t.Fatalf("expected status %d, got %d", http.StatusOK, getRec.Code)
	}

	updateRec := performCommentRequest(t, r, http.MethodPut, resourcePath, map[string]any{"name": "Novels"})
	if updateRec.Code != http.StatusOK {
		t.Fatalf("expected status %d, got %d", http.StatusOK, updateRec.Code)
	}
	updateBody := decodeCommentJSON(t, updateRec.Body.Bytes())
	...
	updatedFilteredRec := performCommentRequest(t, r, http.MethodGet, "/comments/?q=nov", nil)
	if updatedFilteredRec.Code != http.StatusOK {
		t.Fatalf("expected status %d, got %d", http.StatusOK, updatedFilteredRec.Code)
	}
	updatedFilteredBody := decodeCommentJSON(t, updatedFilteredRec.Body.Bytes())
	if got := int(updatedFilteredBody["count"].(float64)); got != 1 {
		t.Fatalf("expected filtered count 1 after update, got %d", got)
	}

	deleteRec := performCommentRequest(t, r, http.MethodDelete, resourcePath, nil)
	if deleteRec.Code != http.StatusNoContent {
		t.Fatalf("expected status %d, got %d", http.StatusNoContent, deleteRec.Code)
	}

	finalListRec := performCommentRequest(t, r, http.MethodGet, "/comments/", nil)
	finalListBody := decodeCommentJSON(t, finalListRec.Body.Bytes())
	if got := int(finalListBody["count"].(float64)); got != 1 {
		t.Fatalf("expected list count 1 after delete, got %d", got)
	}

	badIDRec := performCommentRequest(t, r, http.MethodGet, "/comments/not-a-number", nil)
	assertStructuredErrorResponse(t, badIDRec, http.StatusBadRequest, "BAD_REQUEST")

	missingRec := performCommentRequest(t, r, http.MethodGet, resourcePath, nil)
	assertStructuredErrorResponse(t, missingRec, http.StatusNotFound, "NOT_FOUND")
}

func TestCommentHandler_RejectsInvalidPayload(t *testing.T) {
	repository := repositories.NewCommentRepository()
	service := services.NewCommentService(repository)
	h := NewCommentHandler(service)
	r := router.NewMux()
	...
	rec := performCommentRequest(t, r, http.MethodPost, "/comments/", map[string]any{"name": "  "})
	assertStructuredErrorResponse(t, rec, http.StatusBadRequest, "BAD_REQUEST")
}

func performCommentRequest(t *testing.T, handler http.Handler, method, path string, payload map[string]any) *httptest.ResponseRecorder {
	t.Helper()
...
func decodeCommentJSON(t *testing.T, raw []byte) map[string]any {
	t.Helper()
...
func assertStructuredErrorResponse(t *testing.T, rec *httptest.ResponseRecorder, status int, code string) {
	...
	body := decodeCommentJSON(t, rec.Body.Bytes())
