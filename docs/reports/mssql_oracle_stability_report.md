...
## Post-ADR-004-sprint drill (queued, 2026-05-14)

The ADR-004 integration sprint (PRs #51–#54, merged 2026-05-13) touched `pkg/auth`, `pkg/authz`, `pkg/mail`, `pkg/storage`, and `pkg/app`. None of these directly touch the MSSQL/Oracle driver paths (`pkg/db/driver_{mssql,oracle}.go`) or the migrator dialect plumbing. However, the default-deny middleware now mounted on `Router.Use` flows through every request, including the `/health` paths exercised by the live DB jobs.

A confirmation drill is **queued** but not yet executed:
...
- `gh auth status` succeeds locally (or `GH_TOKEN` exported in CI).
- The workflow supports `workflow_dispatch` (verified — `.github/workflows/ci.yml` declares `on: workflow_dispatch:`).
- The drill is run **against `main`**, not against a worktree branch.

Expected outcome: ≥ 9/10 success on both lanes. Anything below the 80% threshold while no driver code changed signals an interference from one of the four sprint PRs — most likely the default-deny middleware refusing an internal probe route, or the circuit breaker tripping on the SMTP/S3 stub used by the CLI tests.

Owner sign-off required before triggering — the drill dispatches 10 CI runs on `main` and burns ~30 minutes of wall clock plus shared CI minutes. The audit at `docs/audits/2026-05-14-post-sprint-readiness.md` §6 retains MSSQL/Oracle at `usable but thin` pending this drill.
