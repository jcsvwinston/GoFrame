...
| `pkg/openapi` | `experimental` | Minimal OpenAPI 3.1 document model and JSON serialization helpers | Official experimental base layer for project-level API contracts via `internal/contracts` and `goframe openapi`; schema shape may expand before `v1.0`. |
