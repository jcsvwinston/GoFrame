...
| `pkg/openapi` | `experimental` | Minimal OpenAPI 3.1 document model, JSON serialization helpers, and runtime document handler helpers | Official experimental base layer for project-level API contracts via `internal/contracts`, `goframe openapi`, and explicit runtime serving; schema shape may expand before `v1.0`. |
