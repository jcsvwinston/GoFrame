...
| `pkg/openapi` | `experimental` | Minimal OpenAPI 3.1 document model, JSON serialization helpers, runtime document handler helpers, and small schema/response/parameter helpers for scaffolded contracts | Official experimental base layer for project-level API contracts via `internal/contracts`, `goframe openapi`, and explicit runtime serving; the current helper subset covers repeated JSON schema shapes, structured JSON error responses, empty responses, and explicit path/query parameters, but the overall surface may still expand before `v1.0`. |
