...
multitenant:
  enabled: true
  resolution: subdomain   # Options: subdomain, header
  header_name: X-Tenant-ID # Only used when resolution=header
  ...
  tenant_db_mappings:
    ...
    - tenant: "globex"
      database: "globex_db"
    - tenant: "initech"
      database: "shared_db"  # Rejected if require_isolated_db=true
```
...
```yaml
multitenant:
  tenant_db_template: "tenant_{tenant}_db"
```

This maps:
- `acme` -> `tenant_acme_db`
- `globex` -> `tenant_globex_db`
