...
- `.github/workflows/ci.yml` updated so the new live tests are actually exercised by CI. While there, the `TestSQLMatrix_AutoMigrate` and `TestSQLMatrix_AutoMigrate_Exploratory` tests added in PR #63 are also added to the workflow — they had been compiling but never executing because the existing workflow `-run` regex did not pick them up.
